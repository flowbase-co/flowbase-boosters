...
const beforeAfterBooster = new Booster.Booster<
...
>({
  ...
  apply(element, data) {
    ...
    const initThumbSize = () => {
      element.style.setProperty('--thumb-width', `${handleEl.clientWidth}px`)
      element.style.setProperty('--thumb-height', `${handleEl.clientHeight}px`)
    }

    const setPosition = (value: number) => {
      element.style.setProperty('--thumb-position', `${value}%`)
...
export const BeforeAfterFlowbase = () => {
  // Global styles
